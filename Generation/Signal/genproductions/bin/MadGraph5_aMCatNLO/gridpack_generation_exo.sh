...
if [ -n "$5" ]
  then
    ...
    echo "Please provide a width as the 5th argument. Note that 3rd and 4th arugments must also be applied"
    exit
fi
if [ -n "$6" ]
  then
...
#   export RELEASE=CMSSW_5_3_32_patch3
  ...
  #############################################
  #Copy, Unzip and Delete the EXO model#
  #############################################
  cd models
  mkdir pions_from_unification_UFO
  cd pions_from_unification_UFO
  cp  /afs/cern.ch/work/j/jkunkle/public/pi3.tar.gz .
  tar xzf pi3.tar.gz
  rm pi3.tar.gz

  cd ../..

  patch -l -p0 -i $PRODHOME/dynamic_patches/parameters${width}percent.patch
