...
parser.add_argument( '--tchain', dest='tchain',action="store_true",help='tchain mode' )
...
#        print fname
    ...
    if options.tchain:
        ...
        for f in ntuple_files_run:
            mychain.AddFile( f )
            print f, mychain.GetEntries()
        ntuple_files_run=[0] #only loop once for tchain mode

    for f in ntuple_files_run:
        if not options.tchain:
        ...
        print f,
        for i in range(4):
            print weighthist.GetBinContent(i),
        print
