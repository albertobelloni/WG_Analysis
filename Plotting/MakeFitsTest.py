...
parser.add_argument( '--baseDir',       dest='baseDir',
        required=True, help='path to workspace directory' )
parser.add_argument( '--outputDir',     dest='outputDir',
        required=False, default=None, help='name of output diretory for cards')
parser.add_argument( '--doStatTests',   dest='doStatTests',
        action='store_true', help='run statistical tests of WGamma background')
parser.add_argument( '--doWJetsTests',  dest='doWJetsTests',
        action='store_true', help='run tests of Wjets background' )
parser.add_argument( '--doFinalFit',    dest='doFinalFit',
        action='store_true', help='run fit to data and background' )
parser.add_argument( '--doVarOptimization', dest='doVarOptimization',
        action='store_true', help='run variable optimization' )
parser.add_argument( '--doJetOptimization', dest='doJetOptimization',
        action='store_true', help='run jet veto optimization' )
parser.add_argument( '--useHistTemplates',  dest='useHistTemplates',
        action='store_true', help='use histogram templates' )
parser.add_argument( '--useToySignal',      dest='useToySignal',
        action='store_true', help='use gaussian as signal' )
parser.add_argument( '--useToyBackground',  dest='useToyBackground',
        action='store_true', help='use exponential as background ' )
parser.add_argument( '--noRunCombine',      dest='noRunCombine',
        action='store_true', help='Dont run combine, use existing results' )
parser.add_argument( '--combineDir',        dest='combineDir',
        default=None,        help='path to combine directory' )
...
class MakeLimits( ) :
    ...
    def setup( self , kine_vars ):
        ...
        for ibin in self.bins :
            ...
            self.generate_toy_data( kine_vars, sigpars = ['M900_W5_mu'], signorms = [2.0], data_norm=20.0)
        ...
        for width in self.widthpoints:
            ...
            for mass in self.masspoints:
                ...
                for ibin in self.bins:
                    ...
                    sigpar = "_".join( ['M'+ str(mass), 'W'+width, ibin['channel']] )
    ...
    def generate_card( self, outputCard, sigpar, kine_vars, ibin, tag='base' ) :
        ...
        for bkgname, bkg in self.backgrounds.iteritems() :
            ...
            card_entries.append( 'shapes %s %s %s %s:%s' %( bkgname.ljust( max_name_len ),
                bin_id, bkg.outputfname.ljust( max_path_len ), bkg.GetWSName(),
                bkg.GetPDFName( kine_vars[0]['name'], ibin['channel'] ) ) )
    ...
    def prepare_background_functions( self, kine_vars ) :
        ...
        for bkgn in self.bkgnames:
            ...
            print  '%s/bkgfit/2016/%s' %( self.baseDir, self.wskeys[bkgn].GetRootFileName() )
            ofile = ROOT.TFile.Open( '%s/bkgfit/2016/%s' %( self.baseDir, self.wskeys[bkgn].GetRootFileName() ), 'READ' )
            ...
            for var in kine_vars :
                for ibin in self.bins :
                    ...
                    if options.useHistTemplates :
                        ...
                        for ipar in self.wskeys[bkgn].GetParNames(  ibin['channel'] ):
                            if DEBUG:
                               print ipar
            ...
            for width in self.widthpoints:
                for var in kine_vars :
                    for ibin in self.bins :
                        ...
                        sigpar = "_".join(['M'+str(mass), 'W'+str(width), ibin['channel']])

                        ## get the cross section and scale factor information
                        scale = self.weightMap['ResonanceMass%d'%mass]['scale']

                        print '%s/sigfit/workspace_%s_%s.root' %( self.baseDir, self.signame, sigpar )
                        ifile = ROOT.TFile.Open( '%s/sigfit/2016/ws%s_%s.root' %( self.baseDir, self.signame, sigpar ), 'READ' )

                        if DEBUG:
                           print "ws" + self.signame + '_' + sigpar

                        ws_in = ifile.Get( "ws" + self.signame + '_' + sigpar )

                        ws_out = ROOT.RooWorkspace( "workspace_" + self.signame + '_' + sigpar )

                        sigfitparams = OrderedDict()

                        suffix = sigpar+"2016" #"_".join([sigpar, "2016"]) ##FIXME
                        #cb_MadGraphResonance_M300_W5_mu_mt_res_base_EB
