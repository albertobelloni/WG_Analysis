            ...
            for iyear in Years:
                for ich in CH:
                    ...
                    for mass in signal_masses:
                        ...
                        if('norm' in ipar):
                            if("%s_%s_%s"%(str(mass),wid,iyear) in N_tot ): ntot= N_tot["%s_%s_%s"%(str(mass),wid,iyear)]
                            (parlist[grname]).append( ws_in.var(grname.replace("Mass",'M'+str(mass))).getVal())#*50000/ntot )
                            parlists[grname][mass] = ws_in.var(grname.replace("Mass",'M'+str(mass))).getVal()#*50000/ntot
                        else:
                ...
                for wid in signal_widths:
                    for iyear in Years:
                        for ich in CH:
                            ...
                            if 'norm' in ipar:
                                a= ROOT.TF1("a","[0] - [1]*TMath::Exp(-x/[2])",0,2000);
                                ...
                                a= ROOT.TF1("a","[0] + [1]*TMath::Exp(-x/[2])",300,2000);
                                if wid == '5':
                                    a.SetParameters(0.3, 1, 300)
                                if wid == '0p01':
                                    a.SetParameters(0.1, 0.3, 300)
                                gr[grname].Fit("a","R")
...
# ---------------------------------------------------
    ...
    #masspoints = [325, 375, 425, 475, 500, 650, 750, 850, 950, 1100, 1300, 1500, 1700, 1900]

    for mass in masspoints:
        for width in widthpoints:
            for iyear in yearpoints :
                for ich in chpoints :
                    prepare_signal_functions_helper( mass, width, iyear, ich , Use_scaleError=False)
