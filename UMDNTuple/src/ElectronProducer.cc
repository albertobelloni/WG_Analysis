...
void ElectronProducer::initialize( const std::string &prefix,
    ...
    if( detail > 0 ) {
        ...
        if( detail > 1 ) {
            ...
            tree->Branch( (prefix + "_dPhiClusterRack").c_str(), &el_dPhiClusterRack );
    ...
    for (unsigned int j=0; j < electrons->size();++j){
        ...
        if(_detail > 0 ) {
            ...
            if( _detail > 1 ) {
                ...
                el_dPhiClusterRack -> push_back( el->deltaPhiSuperClusterTrackAtVtx());
